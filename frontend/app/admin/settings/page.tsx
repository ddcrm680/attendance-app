    ...
    <section className="space-y-5">
      ...
      {form && !loading && (
        <form onSubmit={submit} className="app-card grid max-w-4xl gap-4 p-4 sm:grid-cols-2">
          ...
          <NumberField label="Location tracking interval (seconds)" value={form.location_tracking_interval_seconds} onChange={(value) => setValue("location_tracking_interval_seconds", value)} />
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="text-sm">{label}<input required type="number" value={value} onChange={(event) => onChange(Number(event.target.value))} className="app-form-control mt-1" /></label>;
}
