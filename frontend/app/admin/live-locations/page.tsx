...
export default function LiveLocationsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      setEmployees(await adminLiveEmployees());
    } catch (cause) {
      setError(
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <p className="text-sm text-gray-500">Loading live locations…</p>
      ) : (
        <>
          <LiveLocationMap employees={employees} />
          <div className="space-y-2">
            {employees.map((employee) => {
              const stale =
                !employee.last_location ||
                Date.now() -
                  new Date(employee.last_location.recorded_at).getTime() >
                  STALE_AFTER_MS;
              return (
                <div
                ...
                >
                  <div className="flex justify-between">
                    ...
                    >
                      {stale ? "Location stale/unavailable" : "Live"}
                    </span>
                  </div>
                  <p className="mt-1 text-gray-500">
                    {employee.office ?? "No office"} · checked in{" "}
                    {new Date(employee.check_in).toLocaleTimeString()}
                  </p>
                  {employee.last_location && (
                    <p className="text-xs text-gray-500">
                      Accuracy {Math.round(employee.last_location.accuracy)}m ·
                      updated{" "}
                      {new Date(
