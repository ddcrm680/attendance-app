...
import { formatDateTime, formatOfficeDistance } from "@/lib/presentation";
...
function EmployeeLocationCard({ employee, selected, onSelect }: { employee: LiveEmployee; selected: boolean; onSelect: () => void }) {
  ...
  return <article className={`live-location-employee-card ${selected ? "live-location-employee-card-selected" : ""}`} tabIndex={0} role="button" aria-label={`Show details for ${employee.name}`} aria-pressed={selected} onClick={onSelect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(); } }}><div className="live-location-employee-card-top"><div><h3>{employee.name}</h3><p>{employee.employee_code}</p></div><span className={`live-location-state ${stale ? "live-location-state-stale" : "live-location-state-live"}`}><i aria-hidden="true" />{stateLabel}</span></div><dl className="live-location-details"><div><dt>Attendance</dt><dd>{employee.status}</dd></div><div><dt>Office</dt><dd>{employee.office ?? "Not available"}</dd></div><div><dt>Checked in</dt><dd>{formatDateTime(employee.check_in)}</dd></div><div><dt>Last updated</dt><dd>{location ? formatDateTime(location.recorded_at) : "No location received"}</dd></div>{location && <><div><dt>Accuracy</dt><dd>{Math.round(location.accuracy)} m</dd></div><div><dt>Distance from office</dt><dd>{formatOfficeDistance(employee.current_distance_meters, employee.mode)}</dd></div><div className="live-location-coordinates"><dt>Coordinates</dt><dd>{formatCoordinates(location.latitude, location.longitude)}</dd></div></>}</dl></article>;
}
