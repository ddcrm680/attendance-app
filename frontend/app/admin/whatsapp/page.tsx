...
export default function WhatsAppAdminPage() {
  ...
  return (
    <section className="space-y-5">
      ...
      {settings && (
        <div className="grid gap-3 rounded border bg-white p-4 text-sm sm:grid-cols-2">
          <p>
          ...
          <p>
            <b>Events:</b>{" "}
            {Object.entries(settings.attendance_notifications)
      ...
      <div className="overflow-x-auto rounded border bg-white">
          ...
          <tbody>
            {logs.map((log) => (
              <tr className="border-t" key={log.id}>
                ...
                <td className="p-3">{log.provider_message_id ?? "—"}</td>
                <td className="p-3">
        ...
        {loading && (
          <p className="p-4 text-sm text-gray-500">Loading delivery logs…</p>
        )}
      </div>
    </section>
  );
