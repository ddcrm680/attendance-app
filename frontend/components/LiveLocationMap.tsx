...
export default function LiveLocationMap({
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-slate-50 p-3">
      ...
      >
        {points.map((employee) => (
          <g key={employee.attendance_id}>
            <circle
              cx={x(employee.last_location!.longitude)}
              cy={y(employee.last_location!.latitude)}
              r="2.5"
              className="fill-blue-600"
            />
            <text
              x={x(employee.last_location!.longitude) + 3}
              y={y(employee.last_location!.latitude) - 3}
              className="fill-slate-700 text-[4px]"
            >
