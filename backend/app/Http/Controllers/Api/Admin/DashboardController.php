...
class DashboardController extends Controller
{
    public function stats()
    {
        ...
        $attendance = Attendance::where('attendance_date', $today)
            ->selectRaw(
                "sum(case when status in ('present','late','half_day','partial','work_from_home') then 1 else 0 end) as present, "
                ."sum(case when status = 'late' then 1 else 0 end) as late, "
                ."sum(case when check_in is not null and check_out is null then 1 else 0 end) as currently_working, "
                ."sum(case when check_out is not null then 1 else 0 end) as checked_out, "
                ."avg(case when working_minutes > 0 then working_minutes end) as average_working_minutes"
            )
            ->first();
    ...
    public function liveEmployees()
    {
        ...
        $working = Attendance::query()
            ->select(
                'attendance.*',
                'latest_location.latitude as live_latitude',
                'latest_location.longitude as live_longitude',
                'latest_location.accuracy as live_accuracy',
                'latest_location.recorded_at as live_recorded_at'
            )
    ...
    public function charts()
    {
        ...
        return response()->json([
            'daily' => Attendance::selectRaw(
                'attendance_date as date, count(*) as total, '
                .'sum(case when status in ("present","late","half_day","partial","work_from_home") then 1 else 0 end) as present, '
                .'sum(case when status = "late" then 1 else 0 end) as late'
            )
