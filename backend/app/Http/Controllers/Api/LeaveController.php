...
class LeaveController extends Controller
{
    public function types()
    {
        return response()->json(
            LeaveType::where('active', true)->orderBy('name')->get()
        );
    }
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            'leave_type_id' => [
                'required',
                Rule::exists('leave_types', 'id')->where('active', true),
            ],
        ...
        if ($type->reason_required && ! filled($data['reason'])) {
            throw ValidationException::withMessages([
                'reason' => ['A reason is required for this leave type.'],
            ]);
        }
    ...
    {
        abort_unless($leave->employee_id === $request->user()->id, 403);
